...
Persists the enriched rows in a physical Gold table via a **Data Flow** (Fabric Data Pipeline +
Notebook) so **Activator and Power BI read a stored table**, and history is retained for
dashboards, backtesting, and Trigger-Amount percentile calibration. Parent: [Gold EWI README](README.md).

## Flow

```
Silver: DepositMovementClassified → Gold_EarlyWarning()  ◄── EWI_AlertThreshold
      → [Data Flow: 30-min scheduled .set-or-append] → DepositMovementEarlyWarning (Gold table)
      → Power BI   +   KQL Query → Data Activator
```

## Why a Data Flow (not a materialized view)
...
is a **function**, and the table is filled by a **scheduled Data Flow** that runs it and appends
new rows — the same pattern as [Production 07 — Interval Scheduler](../07-interval-scheduler/).
...
| Idempotent 30-min append (Data Flow) | [kql/15-append-DepositMovementEarlyWarning.kql](kql/15-append-DepositMovementEarlyWarning.kql) |
