...
time. The function is **self-contained** — it reads Silver directly and joins the
`EWI_AlertThreshold` reference table to assign `Alert_Level`. Parent: [Gold EWI README](README.md).

## Flow

```
Silver: DepositMovementClassified → Gold_EarlyWarning()  ◄── EWI_AlertThreshold
      → Data Activator (runs the function; watches Alert_Level)
```
